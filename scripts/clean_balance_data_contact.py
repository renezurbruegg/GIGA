...
def main(args):
    ...
    # print
    df = read_df_candidates(root)
    positives = df[(df[[f"label_{i}" for i in range(12)]] > 0).any(1)]
    negatives = df[~(df[[f"label_{i}" for i in range(12)]] > 0).any(1)]
    ...
    # print
    df = read_df_candidates(root)
    positives = (df[[f"label_{i}" for i in range(12)]] > 0).any(1)
    negatives = ~(df[[f"label_{i}" for i in range(12)]] > 0).any(1)
